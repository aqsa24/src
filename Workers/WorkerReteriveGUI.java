...
import WorkerDatabase.WorkerDatabase;
...
public class WorkerReteriveGUI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					WorkerReteriveGUI  frame = new WorkerReteriveGUI ();
...
private JLabel lblReterivePatientRecord;
	 ...
	 /* Create the frame.
	 */
	public WorkerReteriveGUI () {
		setType(Type.UTILITY);
		setTitle("Retrieve Patient Record");
		connection=WorkerDatabase.getcon();
		...
		btnLoadTable.setBounds(506, 320, 114, 39);
		contentPane.add(btnLoadTable);
		
		scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 90, 610, 221);
		...
		btnChart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					String sql = "Select ID,Pay from worker";
					JDBCCategoryDataset dataset  = new JDBCCategoryDataset(WorkerDatabase.getcon(),sql);
					JFreeChart chart = ChartFactory.createLineChart("Worker Description", "ID", "Pay", dataset, PlotOrientation.VERTICAL, false, true, true);
		...
		btnChart.setBounds(506, 370, 114, 39);
		...
		textField.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent arg0) {
				try {
					...
					String query = "select * from worker where "+selection+" = ?";
					PreparedStatement pst = connection.prepareStatement(query);
		...
		btnBack.setBounds(10, 370, 114, 39);
		...
		lblReterivePatientRecord = new JLabel("Reterive Worker Record");
